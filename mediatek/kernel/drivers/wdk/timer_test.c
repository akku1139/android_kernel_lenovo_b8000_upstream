...
static generic_timer_ppi_check(void)
{
	disable_cpuxgpt();
...
static cpuxgpt_X_interrupt_test(int cpu)
{
	...
	//
	cpu_xgpt_set_cmp(cpu,0x80000001,0x00000100);
...
static void cpuxgpt_frequency_test(void)
{
	...
	cpu_xgpt_set_cmp(0,0x00000000,TIMEOUT);
	cpuxgpt_t1 = sched_clock();
	set_cpuxgpt_clk(CLK_DIV1);
	enable_cpuxgpt();
...
static void cpuxgpt_halt_on_debug_test(void)
{
	disable_cpuxgpt();
	cpu_xgpt_set_init_count(0x00000000,0x00000000);
	set_cpuxgpt_clk(CLK_DIV1);
...
static void cpuxgpt_run_case(int testcase)
{
	switch(testcase)
	{
		case 0:
